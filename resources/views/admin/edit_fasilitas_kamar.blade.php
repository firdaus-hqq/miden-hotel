        ...
        <- Kembali </a>
            <div class="card card-brown shadow mt-3 mb-5 rounded-20">
                <div class="card-body p-4">
                    <form action="/fasilitas_kamar/{{ $fasilitas_kamar->id }}" method="POST" enctype="multipart/form-data">
                        @method('put')
                        @csrf
                        <div class="mb-3">
                            ...
                            <select class="form-select rounded-10 form-brown text-form-brown" name="kamar_id" id="kamar_id">
                               @foreach ($kamar as $kamar)
                                  @if (old('kamar_id', $fasilitas_kamar->kamar_id) == $kamar->id)
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="fasilitas"
                                ...
                                id="fasilitas" required autofocus value="{{ old('fasilitas', $fasilitas_kamar->fasilitas) }}">
                            @error('fasilitas')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" name="jumlah"
                                ...
                                id="jumlah" required autofocus value="{{ old('jumlah', $fasilitas_kamar->jumlah) }}">
                            @error('jumlah')
                                <div class="invalid-feedback">
                                    {{ $message }}
                                </div>
                        ...
                        <button type="submit" class="btn bg-brown text-white float-end rounded-10 mt-4">Edit Fasilitas</button>
                    </form>
                </div>
            </div>
        @endsection
